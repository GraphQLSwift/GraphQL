...
//      }
  ...
  guard !isNullish(value), let value = value else {
    return []
  }
  ...
  if let type = type as? GraphQLInputObjectType {
    ...
    for (fieldName, _) in fields {
        let newErrors = try isValidValue(value: value[fieldName], type: fields[fieldName]!.type).map {
